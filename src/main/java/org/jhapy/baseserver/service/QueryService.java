...
import org.jhapy.dto.searchCriteria.filter.Filter;
import org.jhapy.dto.searchCriteria.filter.RangeFilter;
...
public abstract class QueryService<ENTITY> {
  ...
  protected <X> Specification<ENTITY> buildSpecification(Filter<X> filter,
      SingularAttribute<? super ENTITY, X>
          field) {
  ...
  protected <OTHER, MISC, X extends Comparable<? super X>> Specification<ENTITY> buildReferringEntitySpecification(
    ...
    if (filter.getSpecified() != null) {
      // Interestingly, 'functionToEntity' doesn't work, we need the longer lambda formula
      result = result
          .and(byFieldSpecified(root -> functionToEntity.apply(root), filter.getSpecified()));
    }
    if (filter.getNotEquals() != null) {
  ...
  protected <X> Specification<ENTITY> notEqualsSpecification(
    ...
    return (root, query, builder) -> builder
        .not(builder.equal(metaclassFunction.apply(root), value));
  }
  ...
  protected Specification<ENTITY> likeUpperSpecification(
      ...
      final String value) {
    return (root, query, builder) -> builder
        .like(builder.upper(metaclassFunction.apply(root)), wrapLikeQuery(value));
  }
  ...
  protected <X extends Comparable<? super X>> Specification<ENTITY> greaterThanOrEqualTo(
      ...
      final X value) {
    return (root, query, builder) -> builder
        .greaterThanOrEqualTo(metaclassFunction.apply(root), value);
  }
  ...
  protected <X extends Comparable<? super X>> Specification<ENTITY> lessThanOrEqualTo(
      ...
      final X value) {
    return (root, query, builder) -> builder
        .lessThanOrEqualTo(metaclassFunction.apply(root), value);
  }
